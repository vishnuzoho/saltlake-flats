/**
 * Velopipe Core Strategy Pipeline Engine - '26
 * Regenerated single-file implementation
 *
 * - Floating "scroll-top" appears when user scrolls past 300px.
 * - Floating click scrolls up to header (#live-header-anchor), not the splash.
 * - Defensive initial-hide CSS prevents FOUC; engine removes it when showing.
 * - Rail buttons (left nav) handled separately; display types are correct.
 */
...
const hideFloatingScrollTop = () => {
  ...
  // re-apply guard to avoid flash before next engine decision
...
const hideScrollTopDefensive = () => {
  // ensure initial hide rule exists so there is no flash if JS hasn't completed yet
  ensureInitialFloatingHide();

  // Target ONLY the floating button to keep nav icons visible
  const selector = FLOATING_BTN_SELECTOR;
  document.querySelectorAll(selector).forEach(btn => {
    // explicitly hide floating display to avoid flash if needed
...
const showScrollTopDefensive = () => {
  // remove the initial hide override before showing
  removeInitialFloatingHide();

  // Target ONLY the floating button
  const selector = FLOATING_BTN_SELECTOR;
  document.querySelectorAll(selector).forEach(btn => {
    applyVisibleStyles(btn, false);
  });
...
const forceShowNavOnMainPage = () => {
  ...
  const splashInView = splashWrapper ? (splashWrapper.getBoundingClientRect().bottom > 50) : (scrollPos < 50);
...
const runVisibilityEngine = () => {
  ...
  // Determine the dynamic threshold by accounting for the splash screen height
  const splashWrapper = document.getElementById('dashboard-splash-wrapper');
  const splashHeight = splashWrapper ? splashWrapper.offsetHeight : 0;
  const dynamicThreshold = splashHeight + SHOW_THRESHOLD_PX;

  // Compute whether splash is considered in view (so we can sync CSS class)
  const splashInView = splashWrapper ? (splashWrapper.getBoundingClientRect().bottom > 50) : (scrollPos < 50);

  // Keep the CSS class synced to JS state so initial paint and runtime match
  try {
    document.body.classList.toggle('splash-in-view', !!splashInView);
  } catch (e) { /* ignore in non-standard environments */ }

  // below threshold -> hide floating defensively (rails remain independent)
  ...
  // if an accordion card is open and visible on-screen, hide the floating button to defend screen space
  ...
  // otherwise safe to show floating and rails
  log('showing floating + rails');
  showFloatingScrollTop();
  // ensure rails are also shown properly (this won't break floating display because floating already set)
  showScrollTopDefensive();
  forceShowNavOnMainPage();
};

// Scroll event throttle via rAF
const onScroll = () => {
  if (!rafScheduled) {
    rafScheduled = true;
